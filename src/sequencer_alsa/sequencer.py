...
class Sequencer(object):
    __ARGUMENTS__ = {
        ...
        'alsa_sequencer_stream':SND_SEQ_OPEN_DUPLEX,
        'alsa_sequencer_mode':SND_SEQ_NONBLOCK,
        'alsa_sequencer_type':'default',
        'alsa_port_name':'__port__',
        'alsa_port_caps':SND_SEQ_PORT_CAP_READ,
        'alsa_port_type':SND_SEQ_PORT_TYPE_MIDI_GENERIC,
    ...
    def set_nonblock(self, nonblock=True):
        if nonblock:
            self.alsa_sequencer_mode = SND_SEQ_NONBLOCK
        else:
            self.alsa_sequencer_mode = 0
        snd_seq_nonblock(self.client, self.alsa_sequencer_mode)

    def get_nonblock(self):
        if self.alsa_sequencer_mode == SND_SEQ_NONBLOCK:
            return True
        else:
            return False

    def _error(self, errcode):
        strerr = snd_strerror(errcode)
        msg = "ALSAError[%d]: %s" % (errcode, strerr)
        raise RuntimeError(msg)

    def _init_handle(self):
        ret = open_client(self.alsa_sequencer_name,
        ...
        if ret == None:
            ...
            self._error(ret)
        self.client = ret
        self.client_id = snd_seq_client_id(self.client)
        self.output_buffer_size = snd_seq_get_output_buffer_size(self.client)
        self.input_buffer_size = snd_seq_get_input_buffer_size(self.client)
        self._set_poll_descriptors()

    def _init_port(self):
        err = snd_seq_create_simple_port(self.client,
                                            self.alsa_port_name, 
                                            self.alsa_port_caps, 
                                            self.alsa_port_type)
    ...
    def _new_subscribe(self, sender, dest, read=True):
        subscribe = new_port_subscribe()
        if read:
            self.read_sender = sender
            self.read_dest = dest
            snd_seq_port_subscribe_set_sender(subscribe, self.read_sender)
            snd_seq_port_subscribe_set_dest(subscribe, self.read_dest)
        else:
            self.write_sender = sender
            self.write_dest = dest
            snd_seq_port_subscribe_set_sender(subscribe, self.write_sender)
            snd_seq_port_subscribe_set_dest(subscribe, self.write_dest)
        snd_seq_port_subscribe_set_queue(subscribe, self.queue)
        return subscribe

    def _subscribe_port(self, subscribe):
        err = snd_seq_subscribe_port(self.client, subscribe)
        if err < 0: self._error(err)
    ...
    def _init_queue(self):
        err = snd_seq_alloc_named_queue(self.client, self.alsa_queue_name)
        if err < 0: self._error(err)
        self.queue = err
        adjtempo = int(60.0 * 1000000.0 / self.sequencer_tempo)
        init_queue_tempo(self.client, self.queue,
                            adjtempo, self.sequencer_resolution)

    def _control_queue(self, ctype, cvalue, event=None):
        err = snd_seq_control_queue(self.client, self.queue, ctype, cvalue, event)
        if err < 0: self._error(err)
        self.drain()
    ...
    def queue_get_tick_time(self):
        status = new_queue_status(self.client, self.queue)
        snd_seq_get_queue_status(self.client, self.queue, status)
        res = snd_seq_queue_status_get_tick_time(status)
        free_queue_status(status)
        return res

    def queue_get_real_time(self):
        status = new_queue_status(self.client, self.queue)
        snd_seq_get_queue_status(self.client, self.queue, status)
        res = snd_seq_queue_status_get_real_time(status)
        free_queue_status(status)
        return (resequencer_alsa.tv_sec, res.tv_nsec)

    def change_tempo(self, tempo, event=None):
        adjbpm = int(60.0 * 1000000.0 / tempo)
        self._control_queue(SND_SEQ_EVENT_TEMPO, adjbpm, event)
        self.sequencer_tempo = tempo
        return True

    def start_sequencer(self, event=None):
        if not self._queue_running:
            self._control_queue(SND_SEQ_EVENT_START, 0, event)
            self._queue_running = True

    def continue_sequencer(self, event=None):
        if not self._queue_running:
            self._control_queue(SND_SEQ_EVENT_CONTINUE, 0, event)
            self._queue_running = True

    def stop_sequencer(self, event=None):
        if self._queue_running:
            self._control_queue(SND_SEQ_EVENT_STOP, 0, event)
            self._queue_running = False
    
    def drain(self):
        snd_seq_drain_output(self.client)

    def queue_eventlen(self):
        status = new_queue_status(self.client, self.queue)
        snd_seq_queue_status_get_events(status)

    def _set_poll_descriptors(self):
        self._poll_descriptors = client_poll_descriptors(self.client)
    ...
    def drop_output(self):
        snd_seq_drop_output_buffer(self.client)

    def output_pending(self):
        return snd_seq_event_output_pending(self.client)
    ...
    def event_write(self, event, direct=False, relative=False, tick=False):
        ...
        if direct:
            # no scheduling
            seqev.queue = SND_SEQ_QUEUE_DIRECT
        else:
            seqev.queue = self.queue
            seqev.flags &= (SND_SEQ_TIME_STAMP_MASK|SND_SEQ_TIME_MODE_MASK)
            if relative:
                seqev.flags |= SND_SEQ_TIME_MODE_REL
            else:
                seqev.flags |= SND_SEQ_TIME_MODE_ABS
            if tick:
                seqev.flags |= SND_SEQ_TIME_STAMP_TICK
                seqev.time.tick = event.tick
            else:
                seqev.flags |= SND_SEQ_TIME_STAMP_REAL
        ...
        if isinstance(event, midi.SetTempoEvent):
            ...
            seqev.type = SND_SEQ_EVENT_TEMPO
            seqev.dest.client = SND_SEQ_CLIENT_SYSTEM
            seqev.dest.port = SND_SEQ_PORT_SYSTEM_TIMER
            seqev.data.queue.queue = self.queue
            seqev.data.queue.param.value = adjtempo
        ## Note Event
        elif isinstance(event, midi.NoteEvent):
            if isinstance(event, midi.NoteOnEvent):
                seqev.type = SND_SEQ_EVENT_NOTEON
            if isinstance(event, midi.NoteOffEvent):
                seqev.type = SND_SEQ_EVENT_NOTEOFF
        ...
        ## Control Change
            ...
            seqev.type = SND_SEQ_EVENT_CONTROLLER
        ...
        ## Program Change
            ...
            seqev.type = SND_SEQ_EVENT_PGMCHANGE
            seqev.data.control.channel = event.channel
            seqev.data.control.value = event.value
        ## Pitch Bench
        elif isinstance(event, midi.PitchWheelEvent):
            seqev.type = SND_SEQ_EVENT_PITCHBEND
            seqev.data.control.channel = event.channel
        ...
        err = snd_seq_event_output(self.client, seqev)
        if (err < 0): self._error(err)
        self.drain()
        return self.output_buffer_size - err

    def event_read(self):
        ev = event_input(self.client)
        if ev and ev.type in (SND_SEQ_EVENT_NOTEON, SND_SEQ_EVENT_NOTEOFF):
            if ev.type == SND_SEQ_EVENT_NOTEON:
                mev = midi.NoteOnEvent()
                mev.channel = ev.data.note.channel
                mev.pitch = ev.data.note.note
                mev.velocity = ev.data.note.velocity
            elif ev.type == SND_SEQ_EVENT_NOTEOFF:
...
class SequencerHardware(Sequencer):
    ...
    SequencerStream = SND_SEQ_OPEN_DUPLEX
    SequencerType = "hw"
    SequencerMode = 0
    ...
    class Client(object):
        ...
        class Port(object):
            def __init__(self, port, name, caps):
                ...
                self.name = name
                self.caps = caps
                self.caps_read = self.caps & SND_SEQ_PORT_CAP_READ
                self.caps_write = self.caps & SND_SEQ_PORT_CAP_WRITE
                self.caps_subs_read = self.caps & SND_SEQ_PORT_CAP_SUBS_READ
                self.caps_subs_write = self.caps & SND_SEQ_PORT_CAP_SUBS_WRITE
    ...
    def _query_clients(self):
        self._clients = {}
        snd_seq_drop_output(self.client)
        cinfo = new_client_info()
        pinfo = new_port_info()
        snd_seq_client_info_set_client(cinfo, -1)
        # for each client
        while snd_seq_query_next_client(self.client, cinfo) >= 0:
            ...
            while (snd_seq_query_next_port(self.client, pinfo) >= 0):
...
class SequencerRead(Sequencer):
    DefaultArguments = {
      ...
      'sequencer_stream':not SND_SEQ_NONBLOCK,
      'alsa_port_caps':SND_SEQ_PORT_CAP_WRITE | SND_SEQ_PORT_CAP_SUBS_WRITE,
    }
    ...
    def subscribe_port(self, client, port):
        ...
        snd_seq_port_subscribe_set_time_update(subscribe, True)
        #snd_seq_port_subscribe_set_time_real(subscribe, True)
...
class SequencerWrite(Sequencer):
    DefaultArguments = {
      ...
      'sequencer_stream':not SND_SEQ_NONBLOCK,
      'alsa_port_caps':SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_SUBS_READ
    }
